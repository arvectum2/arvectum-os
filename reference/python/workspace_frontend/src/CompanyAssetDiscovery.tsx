  ...
  return <section className="asset-discovery" aria-labelledby="asset-discovery-title">
    ...
    {filtered.length ? <div className="asset-discovery-results">{filtered.map((item) => {
      ...
      return <article className="asset-discovery-card" key={item.version_id}>
        ...
        <div className="asset-discovery-actions"><button type="button" disabled={busy} onClick={() => void retrieve(item, false)}>{busy ? text("Открываем…", "Opening…") : text("Открыть", "Open")}</button><button type="button" className="quiet-button" disabled={busy} onClick={() => void retrieve(item, true)}>{text("Скачать", "Download")}</button>{item.media_type === DOCX ? <button type="button" className="quiet-button" onClick={() => onReuse(item)}>{text("Использовать как шаблон", "Use as template")}</button> : null}</div>
